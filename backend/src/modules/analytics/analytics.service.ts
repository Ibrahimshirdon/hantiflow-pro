...
export async function getInventoryInsights() {
  ...
  const [batchSnap, productExpirySnap] = await Promise.all([
    ...
    db.collection("products").where("expiryDate", "<=", cutoff).get(),
  ]);
  const expiringBatchProductIds = new Set(batchSnap.docs.map((d) => d.data().productId as string));
  const additionalExpiringProducts = productExpirySnap.docs.filter((d) => {
    const p = d.data() as { isActive?: boolean };
    return p.isActive && !expiringBatchProductIds.has(d.id);
  }).length;
